 ...
 * Size:   1200x800px
 *
 * Usage:  npx tsx src/generate-previews.ts
 */
...
type TokenType = 'text' | 'comment' | 'keyword' | 'string' | 'number' | 'function' | 'operator';
...
function tokenize(line: string, language: string): Token[] {
  ...
  while (i < line.length) {
    ...
    if (line.slice(i, i + 2) === '//' && (language === 'javascript' || language === 'rust')) {
      tokens.push({ type: 'comment', value: line.slice(i) });
      break;
    }
...
function renderPreview(language: string, code: string, theme: Theme): Buffer {
  const WIDTH = 1200;
  const HEIGHT = 800;
  const FONT_SIZE = 14;
  ...
  ctx.fillText(`preview-${language}.${language === 'rust' ? 'rs' : language === 'python' ? 'py' : 'js'}`, 80, dotY);
...
function main(): void {
  ...
  const languages = Object.keys(samples) as Array<keyof typeof samples>;
